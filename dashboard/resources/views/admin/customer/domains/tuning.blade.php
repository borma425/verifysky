...
@section('content')
  ...
  <div class="es-card p-5 md:p-6">
    <h1 class="text-2xl font-bold text-white">Protection Tuning for {{ $domain }}</h1>
    <p class="mt-2 text-sm text-sky-100/70">Read-only tuning view inside the admin customer mirror.</p>

    <div class="mt-5 grid gap-4 md:grid-cols-2">
      <div class="rounded-xl border border-white/10 bg-[#202632] p-4">
        <div class="text-[10px] uppercase tracking-[0.18em] text-[#76859C]">Origin Server</div>
        <div class="mt-2 text-lg font-semibold text-white">{{ $originServer ?: 'Not configured' }}</div>
      </div>
      <div class="rounded-xl border border-white/10 bg-[#202632] p-4">
        <div class="text-[10px] uppercase tracking-[0.18em] text-[#76859C]">Security Mode</div>
    ...
    <div class="mt-5 grid gap-4 lg:grid-cols-2">
      <div class="rounded-xl border border-white/10 bg-[#202632] p-4">
        <h2 class="text-lg font-bold text-white">Thresholds</h2>
        <div class="mt-3 space-y-2 text-sm text-sky-100/80">
          ...
          @empty
            <div>No custom thresholds configured.</div>
          @endforelse
        </div>
      </div>
      <div class="rounded-xl border border-white/10 bg-[#202632] p-4">
        <h2 class="text-lg font-bold text-white">Challenge Profiles</h2>
