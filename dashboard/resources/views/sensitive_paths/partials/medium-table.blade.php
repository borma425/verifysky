<div class="es-card es-animate p-0 border border-amber-500/20">
  <div class="flex items-center justify-between border-b border-amber-500/20 bg-amber-500/10 px-4 py-3">
    <div class="flex items-center gap-3">
      <h3 class="text-base font-bold text-amber-100 flex items-center gap-2"><span class="text-xl">Forced Challenge</span></h3>
      <span class="es-chip bg-slate-800 text-xs text-slate-300">{{ count($mediumPaths) }} Paths</span>
    </div>
    <button type="button" class="es-btn es-btn-warning px-3 py-1.5 text-xs hidden js-bulk-medium" id="bulkMediumBtn">Unlock Selected</button>
  </div>
  ...
  <form id="bulkMediumForm" method="POST" action="{{ route('sensitive_paths.bulk_destroy') }}">
    ...
    <div class="overflow-x-auto min-h-[300px]">
      <table class="es-table w-full text-sm">
        <thead>
        <tr>
          <th class="w-8 text-center px-2"><input type="checkbox" id="selectAllMedium" class="rounded border-slate-600 bg-slate-800 focus:ring-amber-500"></th>
          <th>Domain & Path</th>
          <th>Type</th>
        ...
        @forelse($mediumPaths as $path)
          <tr class="align-top hover:bg-white/[0.02]">
            <td class="text-center px-2 py-3"><input type="checkbox" name="path_ids[]" value="{{ $path['id'] }}" class="rule-cb-med rounded border-slate-600 bg-slate-800 focus:ring-amber-500"></td>
            <td class="py-3">
              <div class="mb-1">
            ...
            <td class="text-right pr-4 py-3">
              <button type="button" class="text-slate-400 hover:text-amber-400 transition js-single-unlock" data-path-id="{{ $path['id'] }}" title="Unlock Path">
